...
由于全网没有搜索到相关的数据集，因此为模拟腾讯广告审核业务场景，使用 Python 脚本生成符合真实业务特征的审核日志数据(整个数据集约1000万行)，用于指标分析、看板搭建以及综合绩效数学建模，具体逻辑如下：
(生成逻辑并不能完全覆盖真实业务场景,仅供参考)
### 1. 数据时间与审核员
...
- **审核员属性**：
  ...
  - 韧性因子（resilience_factor）：模拟正态分布，区间 0.3–0.99
...
- 产出得分衡量审核员**做的多不多**
- 在本模型中，产出更多地作为 **门槛指标**：保证审核员完成基础工作量，同时将主要评价权交给 **效率** 和 **质量** 维度  
- **原因**：效率在一定程度上已经反映了审核量——高效率审核员在固定时间内通常产出也高  
-    如果产出和效率都直接对标历史平均，容易出现信息重叠，导致权重失衡  
注:记得使用比例校正法统一历史时间和绩效时间
#### 计算方式
...
- $\bar{Q}_j$：模块 $j$ 的历史平均每人审核量
-  $H_j$ 为模块 $j$ 历史期总审核量
- $R_j$为历史期参与审核的人数  

##### 多模块加权得分

当审核员 $i$ 负责多个模块时，总产出得分为各模块产出得分的**审核量加权平均**：
...
| 模块 | 审核量 $Q_{i,j}$ | 历史平均每人产出 $\bar{Q}_j$ | 模块产出得分 $S_{产出,i,j}$ |
| ---- | --------------- | ---------------------------- | -------------------------- |
| 1    | 50              | 100                          | 0.50                       |
| 2    | 80              | 60                           | 1.00                       |
...
$$
S_{\text{产出}, i} = \frac{50 \cdot 0.50 + 80 \cdot 1.00}{50 + 80} \approx 0.846
$$
...
由于最终绩效得分极值过大,仅考虑使用z-score进行标准化,如果需要归一化可以考虑在各个模块得分最后提前进行归一化

由于生成数据不能完全现实业务情况,因此结果仅供参考

将绩效模型带入生成数据中,并将绩效评价时间设置为2025-09-24至2025-09-30(9月最后一周),取总绩效得分前后20名的数据:

![指标体系](./绩效情况.png)


综合绩效前20名的总审核量基本在1000以上,并且正确率全部在96%以上,北极星指标-每小时正确审核数也都在10以上(北极星指标排名和最终绩效得分排名不匹配的原因是每个人负责的模块难度不同)

而综合绩效后20名的总审核量基本在100左右,严重不符合历史平均产出,正确率也都在50%上下浮动,北极星指标更是只有1左右

因此该绩效模型可以较好地区分出不同水平的审核员

后续优化方向:

1.确认合适的风险补偿系数以此调整难度对绩效的影响-前20名的北极星指标排名和最终绩效排名差异较大

2.调整产出得分权重或模型-前20名审核员中有产出异常高且质量过关但最终得分比较一般的情况
