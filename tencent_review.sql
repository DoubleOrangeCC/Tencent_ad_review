...
with cte as (select reviewer_id                                                              as 审核员id,
                    count(*)                                                                 as 总审核数,
                    sum(review_sec)                                                          as 总耗时,
                    sum(case when is_correct = 1 then 1 else 0 end)                          as 总正确数,
                    sum(case when is_correct = 0 then 1 else 0 end)                          as 总错误数,
                    ...
                    sum(case when review_result = '不通过' then 1 else 0 end)                as 总不通过数,
                    sum(case when review_result = '通过' then 1 else 0 end) / count(*)       as 通过率,
                    sum(case when is_correct = 1 then 1 else 0 end) / sum(review_sec) * 3600 as 每小时正确审核数
             from tencent
             where date between '2025-09-24' and '2025-09-30'
             group by reviewer_id),
     cte2 as (select reviewer_id,
                     50 + (10 * (final_score - avg(final_score) over ()) / std(final_score) over ()) as final_score
              from 绩效模型),
    cte3 as (select cte.*, final_score, row_number () over (order by final_score desc) as rn
    from cte
    join cte2 on cte2.reviewer_id = cte.审核员id)
select * from cte3 where rn <= 20 or rn >=2981 order by rn;
